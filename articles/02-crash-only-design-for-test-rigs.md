...
The non-negotiable rule for a crash-only runner: any state that needs to survive a reboot must be on durable storage before the operation that depends on it begins. Not after. Not eventually. Before. The mechanism is usually `fsync`; the requirement is true durability, the bytes actually on stable media.
...
A couple of those terms: a kernel softlockup is the CPU stuck spinning with interrupts on (a busy hang), a hardlockup is stuck with interrupts off (a deader hang); the kernel detects both. IPMI and Redfish are the two standard protocols for talking to a BMC out-of-band, IPMI the older binary one, Redfish the newer HTTP/JSON one; "hard reset over IPMI/Redfish" just means the BMC is told to power-cycle the host CPU through one of them.
